...
pub fn build_single(
    ...
    for (accession, v) in map {
        ...
        if v.len() == 2 {
            ...
            let filename = &v[0];
            if is_fastq_path(filename) {
                // single-end FASTQ(/gz)
                let unfiltered = kmer::kmers_from_fq_qual(filename.to_owned(), k_size, 1, quality);
                let kmers = if cutoff == -1 {
                    let auto_cutoff = kmer::auto_cutoff(unfiltered.to_owned());
                    kmer::clean_map(unfiltered, auto_cutoff)
                } else {
                ...
                // FASTA (including .fasta.gz, .fa.gz, etc.)
                let vec = kmer::read_fasta(filename.to_string());
                let kmers = if cutoff == -1 {
...
pub fn build_multi(
    ...
    let d: Vec<_>;
    {
            ...
            .map(|l| {
                if l.1.len() == 2 {
                    ...
                    if is_fastq_path(filename) {
                        // single-end FASTQ(/gz)
                        let unfiltered =
                            kmer::kmers_from_fq_qual(filename.to_owned(), k_size, 1, quality);
                        let kmers = if cutoff == -1 {
                            let auto_cutoff = kmer::auto_cutoff(unfiltered.to_owned());
                            kmer::clean_map(unfiltered, auto_cutoff)
                        } else {
        ...
        //create hash table with colors for accessions
        let num_taxa = accessions.len();
        for (c_i, s) in accessions.iter().enumerate() {
            accession_colors.insert(s.to_string(), c_i);
            colors_accession.insert(c_i, s.to_string());
        }
...
pub fn build_multi_mini(
    ...
    let d: Vec<_>;
    {
        ...
        c = map_vec
            ...
            .map(|l| {
                if l.1.len() == 2 {
                    // paired-end FASTQ with minimizers + qualities
                    ...
                    let filename = &l.1[0];
                    if is_fastq_path(filename) {
                        // single-end FASTQ(/gz) with minimizers + qualities
                        ...
                        // FASTA (including .fasta.gz, etc.) with minimizers (no qualities)
                        let vec = kmer::read_fasta(filename.to_string());
                        let kmers = if cutoff == -1 {
...
pub fn build_single_mini(
    ...
    for (accession, v) in map {
        ...
        counter += 1;
        if v.len() == 2 {
            // paired-end FASTQ: get kmers with quality, then take minimizer per kmer
            let unfiltered = kmer::kmers_fq_pe_qual(vec![&v[0], &v[1]], k_size, 1, quality);
            let kmers = if cutoff == -1 {
                let auto_cutoff = kmer::auto_cutoff(unfiltered.to_owned());
                kmer::clean_map(unfiltered, auto_cutoff)
            ...
            let filename = &v[0];
            if is_fastq_path(filename) {
                // single-end FASTQ(/gz): kmers with qual, then minimizers
                let unfiltered = kmer::kmers_from_fq_qual(filename.to_owned(), k_size, 1, quality);
                let kmers = if cutoff == -1 {
                    let auto_cutoff = kmer::auto_cutoff(unfiltered.to_owned());
                    kmer::clean_map(unfiltered, auto_cutoff)
                ...
                // FASTA (including .fasta.gz, etc.): kmers then minimizers
                let vec = kmer::read_fasta(filename.to_string());
                let kmers = if cutoff == -1 {
                    kmer::kmerize_vector(&vec, k_size, 1)
                } else {
